      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <Select value={selectedCategory} onValueChange={setSelectedCategory}>
            <SelectTrigger className="w-48 bg-secondary border-gray-600 text-text">
        ...
        <Card className="neumorphic-card border-0 mb-8">
          <CardHeader>
            <CardTitle className="text-text font-poppins">Compatibility Matrix</CardTitle>
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-text font-medium">Framework/Library</TableHead>
                    {services.map((service) => (
                      <TableHead key={service} className="text-center text-text font-medium min-w-32">
                        {service}
                      </TableHead>
                    ))}
                  </TableRow>
                ...
                <TableBody>
                  {filteredTechnologies.map((tech) => (
                    <TableRow key={tech}>
                      <TableCell className="font-medium text-text">{tech}</TableCell>
                      {services.map((service) => {
        ...
        <Card className="neumorphic-card border-0 mb-8">
          <CardHeader>
            <CardTitle className="text-text font-poppins">Compatibility Legend</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-3 gap-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-text font-medium">Fully Compatible</p>
                  <p className="text-text-secondary text-sm">Works seamlessly without issues</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <AlertTriangle className="w-5 h-5 text-yellow-500" />
                <div>
                  <p className="text-text font-medium">Works with Caveats</p>
                  <p className="text-text-secondary text-sm">Requires additional configuration</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <XCircle className="w-5 h-5 text-red-500" />
                <div>
                  <p className="text-text font-medium">Not Recommended</p>
                  <p className="text-text-secondary text-sm">Known conflicts or limitations</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="neumorphic-card border-0">
          <CardHeader>
            <CardTitle className="text-text font-poppins">Popular Compatible Combinations</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                <div key={index} className="border border-gray-600 rounded-lg p-4 hover:border-accent/50 transition-colors">
                  <h3 className="text-text font-medium mb-2">{combo.name}</h3>
