    ...
    <div className="min-h-screen bg-primary">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-text font-poppins mb-4">Compare Tools & Frameworks</h1>
          <p className="text-text-secondary text-lg">Compare technologies that serve the same purpose</p>
        </div>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {Object.keys(toolCategories).map((cat) => (
              <Button
                ...
                className={category === cat ? "bg-accent text-white" : "border-gray-600 text-text-secondary"}
              >
                {cat}
              </Button>
            ))}
              ...
              <Card key={tool.id} className="neumorphic-card border-0">
                <CardContent className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-lg font-semibold text-text font-poppins">{tool.name}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-text-secondary">{tool.stats.githubStars}</span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <Download className="w-3 h-3 text-green-500" />
                      <span className="text-text-secondary">{tool.stats.npmDownloads}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        {selectedTools.length >= 2 && (
          <div className="space-y-8">
            ...
            <Card className="neumorphic-card border-0">
              <CardHeader>
                <CardTitle className="text-text font-poppins">Performance Radar</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="neumorphic-card border-0">
              <CardHeader>
                <CardTitle className="text-text font-poppins">Popularity Trends (Last 6 Months)</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="neumorphic-card border-0">
              <CardHeader>
                <CardTitle className="text-text font-poppins">Feature Comparison</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-text">Feature</TableHead>
                      {selectedTools.map((tool) => (
                        <TableHead key={tool.name} className="text-text text-center">{tool.name}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {Object.keys(selectedTools[0]?.features || {}).map((feature) => (
                      <TableRow key={feature}>
                        <TableCell className="text-text-secondary">{feature}</TableCell>
                        {selectedTools.map((tool) => (
            ...
            <Card className="neumorphic-card border-0">
              <CardHeader>
                <CardTitle className="text-text font-poppins">Key Statistics</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className="text-text">Metric</TableHead>
                        {selectedTools.map((tool) => (
                          <TableHead key={tool.name} className="text-text text-center">{tool.name}</TableHead>
                        ))}
                      </TableRow>
                    ...
                    <TableBody>
                      <TableRow>
                        <TableCell className="text-text-secondary">GitHub Stars</TableCell>
                        {selectedTools.map((tool) => (
                          <TableCell key={tool.name} className="text-center text-text">{tool.stats.githubStars}</TableCell>
                        ))}
                      </TableRow>
                      <TableRow>
                        <TableCell className="text-text-secondary">Weekly Downloads</TableCell>
                        {selectedTools.map((tool) => (
                          <TableCell key={tool.name} className="text-center text-text">{tool.stats.npmDownloads}</TableCell>
                        ))}
                      </TableRow>
                      <TableRow>
                        <TableCell className="text-text-secondary">Contributors</TableCell>
                        {selectedTools.map((tool) => (
                          <TableCell key={tool.name} className="text-center text-text">{tool.stats.contributors}</TableCell>
                        ))}
                      </TableRow>
                      <TableRow>
                        <TableCell className="text-text-secondary">Open Issues</TableCell>
                        {selectedTools.map((tool) => (
                          <TableCell key={tool.name} className="text-center text-text">{tool.stats.issues}</TableCell>
                        ))}
                      </TableRow>
                    </TableBody>
                  </Table>
