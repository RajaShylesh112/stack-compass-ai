          ...
          {techStacks.map((stack) => (
            <div key={stack.id} className="neumorphic-card p-8 space-y-6 group cursor-pointer">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-[#FFFFFF]">{stack.name}</h3>
                  <p className="text-[#A259FF] text-sm font-medium">{stack.type}</p>
                </div>
              ...
              {/* Technologies */}
              <div className="space-y-3">
                <h4 className="text-[#FFFFFF] font-medium text-sm uppercase tracking-wide">Technologies</h4>
                <div className="flex flex-wrap gap-2">
                  {stack.technologies.map((tech) => (
                    <Badge key={tech} variant="secondary" className="bg-[#2A2A2A] text-[#CCCCCC] border border-[#333333]">
                      {tech}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                <h4 className="text-[#FFFFFF] font-medium text-sm uppercase tracking-wide">Performance Metrics</h4>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-[#CCCCCC] text-sm">Performance</span>
                    <span className="text-[#FFFFFF] font-medium">{stack.performance}%</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[#CCCCCC] text-sm">Popularity</span>
                    <span className="text-[#FFFFFF] font-medium">{stack.popularity}%</span>
                  </div>
                  <div className="w-full bg-[#2A2A2A] rounded-full h-2">
                    <div 
                      className={`h-2 bg-gradient-to-r ${stack.gradient} rounded-full transition-all duration-500 group-hover:animate-pulse`}
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[#CCCCCC] text-sm">Learning Curve</span>
                    <span className="text-[#FFFFFF] font-medium">{stack.learning}%</span>
                  </div>
                  <div className="w-full bg-[#2A2A2A] rounded-full h-2">
                    <div 
                      className={`h-2 bg-gradient-to-r ${stack.gradient} rounded-full transition-all duration-500 group-hover:animate-pulse`}
                ...
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1">
                    {stack.pros.map((pro, index) => (
                      <li key={index} className="text-[#CCCCCC] text-xs flex items-center">
